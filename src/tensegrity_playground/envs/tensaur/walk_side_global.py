...
def default_config() -> config_dict.ConfigDict:
    return config_dict.create(
        ...
        enable_reset_randomization=False,  # 添加这个配置项
        reset_randomization=config_dict.create(
            x_range=[-5.0, 5.0],
            y_range=[-0.2, 0.2],
            yaw_range=[-3.14 / 6, 3.14 / 6],
            qvel_range=[-0.2, 0.2],
        ),
...
class Walk(mjx_env.MjxEnv):
    ...
    def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
        ...
        state.metrics["global_vel_x"] = self.get_global_linvel(data)[0]
        state.metrics["global_vel_y"] = self.get_global_linvel(data)[1]
    ...
    def _reward_forward_progress(self, global_vel: jax.Array) -> jax.Array:
        # 直接取全局 x 方向速度；若想更稳健也可用 (x_{t+1}-x_t)/dt
        vx = global_vel[0]
        return vx 

    def _reward_side_progress(self, vel: jax.Array) -> jax.Array:
        vx = vel[0]
        vy = vel[1]
        return vy - jp.abs(vx)
    ...
    def _cost_yaw(self, data: mjx.Data) -> jax.Array:
        """惩罚机器人朝向偏离全局 X 轴"""
        quat = self.get_orientation(data)  # [qw, qx, qy, qz]
