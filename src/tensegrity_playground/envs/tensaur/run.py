...
class Walk(mjx_env.MjxEnv):
    ...
    def _get_obs(
        ...
        state = jp.hstack(
            [
                ...
                noisy_gyro,                  # 3
                noisy_gravity,               # 3
                noisy_joint_angles - self._default_joint_pos,  # 16
                noisy_joint_vel,             # 16
                info["last_act"],            # nu (16)
        ...
        # state 总维度: 3+3+3+16+16+16+N(vertebra_pos)+N(vertebra_quat)
        ...
        privileged_state = jp.hstack([
            ...
            linvel,                          # 3
            angvel,                          # 3
            joint_angles - self._default_joint_pos,  # 16
            joint_vel,                       # 16
            data.actuator_force,             # nu (16)
        ])
        # privileged_state 总维度: state + 3+3+3+3+3+16+16+16 + N(vertebra_pos)+N(vertebra_quat)
    ...
    def _get_reward(
        ...
        return {
            ...
            # "pose": self._reward_pose(data.qpos[self._joints_qpos_idx]),
    ...
    def _cost_energy(
        ...
        # 每个关节只有1个 position actuator，force 与 joint 一一对应
