...
mode = "uneven"  # "plane" | "uneven" | "uneven_xml"
...
DEFAULT_CONFIG = {
    ...
    "terrain": {
        ...
        # mode="uneven"：
        "custom_xml_path": "generated_terrain.xml",  # 你的地形XML文件路径
        "hfield_png": "rough_terrain.png",
        "rx": 7.5,         # half length （m）  -> (-1,10)
        "ry": 4,         # half width（m）  -> 总宽 3 m
        "hz": 0.3,        # vertical height scale（m）
        "base": 0.01,     # base height offset（m）
...
def add_xml_terrain(model, config):
    """从自定义 XML 文件加载地形。"""
    tcfg = config.get("terrain", {})
    xml_path = tcfg.get("custom_xml_path")
    
    if not xml_path:
        raise ValueError("custom_xml_path is required for uneven_xml terrain mode.")
    
    # 解析 XML 路径
    p = epath.Path(xml_path)
    if not p.is_absolute():
        alt = ROOT_PATH / "xmls" / xml_path
        alt2 = ROOT_PATH / xml_path
        p = alt if alt.exists() else alt2 if alt2.exists() else None
        if not p:
            raise FileNotFoundError(f"Custom terrain XML not found: {xml_path}")
    ...
    # 从 XML 中提取所有 geom 元素并添加到 worldbody
    for geom_elem in root.findall(".//geom"):
        geom_dict = geom_elem.attrib.copy()
        # 将字符串属性转换为适当的类型
        for key in ["pos", "size", "rgba"]:
            if key in geom_dict:
                geom_dict[key] = [float(v) for v in geom_dict[key].split()]
        
        model.worldbody.add("geom", **geom_dict)
...
def generate_quadruped_from_config(config: dict, vis=visual):
    ...
    elif terrain_mode == "uneven_xml":
        add_xml_terrain(model, config)
    else:
...
def build_config_from_genes(genes: dict, individual_id: int, output_path: str | None=None) -> dict:
    ...
    # 输出路径
    cfg["output_path"] = (ROOT_PATH / "xmls" / f"test.xml") if output_path is None else Path(output_path)
    return cfg
...
if __name__ == "__main__":
    example_genes = {
        ...
        "lateral_hori_stiffness": 500,
        "lateral_verti_stiffness": 500,
        "diagonal_stiffness": 500,
    }


    config = build_config_from_genes(example_genes, individual_id=0)
    # config["terrain"]["custom_xml_path"] = "/media/di/4441-E469/tensaur-main/terrain.xml"
    # config["terrain"]["mode"] = "uneven_xml"    
